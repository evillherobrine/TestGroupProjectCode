...
fun UserPlaylistDetailScreen(
    ...
    val songsState: State<List<Song>> = when (playlistId) {
        Playlist.FAVOURITES_PLAYLIST_ID -> {
            val favoritesViewModel: FavoritesViewModel = viewModel()
            favoritesViewModel.favoriteSongs.collectAsState(initial = emptyList())
        }

        Playlist.MOST_PLAYED_PLAYLIST_ID, Playlist.QUICK_MIX_PLAYLIST_ID -> {
            val libraryViewModel: LibraryViewModel = viewModel(factory = LibraryViewModel.Factory(context))
            produceState(initialValue = emptyList(), key1 = playlistId) {
                libraryViewModel.getPlaylistSongs(playlistId) { songs ->
                    value = songs
                }
            }
        }
            ...
            localPlaylistViewModel.songs.collectAsState(initial = emptyList())
        }
    }
